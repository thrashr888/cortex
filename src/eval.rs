...
mod tests {
  ...
  "cases": [
    ...
    #[test]
    fn camel_case_query_matches_spaced_canonical_phrase() {
        let path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("eval")
            .join("benchmark.json");
        let report = run_benchmark_file(&path).unwrap();
        let case = report
            .case_reports
            .iter()
            .find(|case| case.name == "camelCase query should match spaced canonical phrase")
            .unwrap();
        assert_eq!(case.retrieved_project_keys, vec!["temp_file_writer_lock".to_string()]);
        assert!(case.present_forbidden_context.is_empty(), "unexpected case: {case:#?}");
    }
    ...
    #[test]
    fn dotted_query_matches_spaced_canonical_phrase() {
        let path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("eval")
            .join("benchmark.json");
        let report = run_benchmark_file(&path).unwrap();
        let case = report
            .case_reports
            .iter()
            .find(|case| case.name == "dotted query should match spaced canonical phrase")
            .unwrap();
        assert_eq!(case.retrieved_project_keys, vec!["temp_file_writer_lock".to_string()]);
        assert!(case.present_forbidden_context.is_empty(), "unexpected case: {case:#?}");
    }
    ...
    #[test]
    fn slash_delimited_query_matches_spaced_canonical_phrase() {
        let path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("eval")
            .join("benchmark.json");
        let report = run_benchmark_file(&path).unwrap();
        let case = report
            .case_reports
            .iter()
            .find(|case| case.name == "slash-delimited query should match spaced canonical phrase")
            .unwrap();
        assert_eq!(case.retrieved_project_keys, vec!["temp_file_writer_lock".to_string()]);
        assert!(case.present_forbidden_context.is_empty(), "unexpected case: {case:#?}");
    }
    ...
    #[test]
    fn backslash_delimited_query_matches_spaced_canonical_phrase() {
        let path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("eval")
            .join("benchmark.json");
        let report = run_benchmark_file(&path).unwrap();
        let case = report
            .case_reports
            .iter()
            .find(|case| case.name == "backslash-delimited query should match spaced canonical phrase")
            .unwrap();
        assert_eq!(case.retrieved_project_keys, vec!["temp_file_writer_lock".to_string()]);
        assert!(case.present_forbidden_context.is_empty(), "unexpected case: {case:#?}");
    }
    ...
    #[test]
    fn namespace_delimited_query_matches_spaced_canonical_phrase() {
        let path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("eval")
            .join("benchmark.json");
        let report = run_benchmark_file(&path).unwrap();
        let case = report
            .case_reports
            .iter()
            .find(|case| case.name == "namespace-delimited query should match spaced canonical phrase")
            .unwrap();
        assert_eq!(case.retrieved_project_keys, vec!["temp_file_writer_lock".to_string()]);
        assert!(case.present_forbidden_context.is_empty(), "unexpected case: {case:#?}");
    }
    ...
    #[test]
    fn plus_delimited_query_matches_spaced_canonical_phrase() {
        let path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("eval")
            .join("benchmark.json");
        let report = run_benchmark_file(&path).unwrap();
        let case = report
            .case_reports
            .iter()
            .find(|case| case.name == "plus-delimited query should match spaced canonical phrase")
            .unwrap();
        assert_eq!(case.retrieved_project_keys, vec!["temp_file_writer_lock".to_string()]);
        assert!(case.present_forbidden_context.is_empty(), "unexpected case: {case:#?}");
    }
    ...
    #[test]
    fn comma_delimited_query_matches_spaced_canonical_phrase() {
        let path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("eval")
            .join("benchmark.json");
        let report = run_benchmark_file(&path).unwrap();
        let case = report
            .case_reports
            .iter()
            .find(|case| case.name == "comma-delimited query should match spaced canonical phrase")
            .unwrap();
        assert_eq!(case.retrieved_project_keys, vec!["temp_file_writer_lock".to_string()]);
        assert!(case.present_forbidden_context.is_empty(), "unexpected case: {case:#?}");
    }
    ...
    #[test]
    fn semicolon_delimited_query_matches_spaced_canonical_phrase() {
        let path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("eval")
            .join("benchmark.json");
        let report = run_benchmark_file(&path).unwrap();
        let case = report
            .case_reports
            .iter()
            .find(|case| case.name == "semicolon-delimited query should match spaced canonical phrase")
            .unwrap();
        assert_eq!(case.retrieved_project_keys, vec!["temp_file_writer_lock".to_string()]);
        assert!(case.present_forbidden_context.is_empty(), "unexpected case: {case:#?}");
    }
